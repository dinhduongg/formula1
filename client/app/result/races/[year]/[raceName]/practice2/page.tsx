import getPractice2 from '@/lib/getPractice2'
...
export function generateMetadata({ params: { year, raceName } }: Props) {
  return {
    title: `FORMULA 1 ${raceName.trim().replaceAll('-', ' ').toUpperCase()} GRAND PRIX ${year} - PRACTICE 2`,
    description: `This is the page of ${year} ${raceName} PRACTICE 2`
  }
}

export default async function Practice2({ params: { year, raceName } }: Props) {
    const practiceData: Promise<IPractice> = getPractice2(year, raceName)
