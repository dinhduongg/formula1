...
export function generateMetadata({ params: { year, raceName } }: Props) {
  return {
    title: `FORMULA 1 ${raceName.trim().replaceAll('-', ' ').toUpperCase()} GRAND PRIX ${year} - PRACTICE 1`,
    description: `This is the page of ${year} ${raceName} PRACTICE 1`
  }
}

export default async function Practice1({ params: { year, raceName } }: Props) {
    const practice1Data: Promise<IPractice> = getPractice3(year, raceName)
    const practice = await practice1Data
